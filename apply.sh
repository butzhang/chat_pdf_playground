...
original_name="chat_pdf_playground"
original_urlname="chat_pdf_playground"
original_description="Awesome chat_pdf_playground created by butzhang" 
...
rm -rf "${name}"
rm -rf tests
cp -R ${TEMPLATE_DIR}/chat_pdf_playground "${name}"
...
then
    ...
    echo "then run 'chat_pdf_playground' or 'python -m chat_pdf_playground'"
else
    poetry install
    echo "Applied Flask template"
    echo "Ensure you activate your env with 'poetry shell'"
    echo "then run 'chat_pdf_playground' or 'python -m chat_pdf_playground' or 'poetry run chat_pdf_playground'"
fi
