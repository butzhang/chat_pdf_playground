...
- 🎯 Entry points to execute your program using `python -m <chat_pdf_playground>` or `$ chat_pdf_playground` with basic CLI argument parsing.
...
---
# chat_pdf_playground Flask Application

Awesome chat_pdf_playground created by butzhang
...
```bash
git clone https://github.com/butzhang/chat_pdf_playground chat_pdf_playground
cd chat_pdf_playground
make install
```
...
```bash
pip install chat_pdf_playground
```
...
```bash
chat_pdf_playground create-db   # run once
chat_pdf_playground populate-db  # run once (optional)
chat_pdf_playground add-user -u admin -p 1234  # ads a user
chat_pdf_playground run
```
